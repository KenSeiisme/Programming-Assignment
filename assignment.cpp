...
pmtResult pmt_member(string cust_id, int bill_id, histRecord history[], int& history_count)
{
    int customerIndex = findCustomerIndex(cust_id);
    int memberIndex = findMemberIndex(cust_id);
    ...
    // Check whether Customer / Member ID exists
    if (customerIndex == -1 && memberIndex == -1)
    {
        cout << RED << "[Error] Customer / Member ID not found.\n" << RESET;
        return result;
    }
    ...
    cout << "\nThe Member Registration/Renewal fee is RM100.00/MONTH\n";

    // Asking for membership duration
    do
    {
        ...
        {
            cout << RED << "Invalid duration. Please re-enter...\n" << RESET;
            clearInput();
            duration = 0;
        }
    ...
    do
    {
        ...
        if (result.status == true)
        {
            ...
            if (customerIndex != -1)
            {
                if (memberCount >= MAX_MEMBERS)
                {
                    cout << RED << "[Error] Member database is full.\n" << RESET;
                    result.status = false;
                    return result;
                }
                ...
                // Transfer Customer data to Member
                memberDB[memberCount].idMember = newMemberID;
                memberDB[memberCount].nameMember =
                    customerDB[customerIndex].nameCustomer;
                memberDB[memberCount].genderMember =
                    customerDB[customerIndex].genderCustomer;
                memberDB[memberCount].phoneMember =
                    customerDB[customerIndex].phoneCustomer;
                memberDB[memberCount].emailMember =
                    customerDB[customerIndex].emailCustomer;
                memberDB[memberCount].passwordMember =
                    customerDB[customerIndex].passwordCustomer;

                memberCount++;

                // Remove Customer from Customer database
                for (int j = customerIndex; j < customerCount - 1; j++)
                {
                ...
                // Update payment history from old Customer ID
                // to new Member ID
                if (history_count > 0 &&
                    history[history_count - 1].bill_id == bill_id)
                {
                    history[history_count - 1].customer_id = newMemberID;
                }

                cout << GREEN
                    << "\n[System] Successfully upgraded to Member!"
                    << RESET << endl;

                cout << "New Member ID: " << newMemberID << endl;
            ...
            // ==========================================
            // Existing Member -> Membership Renewal
            // ==========================================
            else if (memberIndex != -1)
            {
                cout << GREEN
                    << "\n[System] Membership renewed successfully!"
                    << RESET << endl;

                cout << "Member ID: " << cust_id << endl;
                cout << "Renewal Duration: " << duration << " month(s)" << endl;
